...
pnpm test:dsp         # fast unit tests: DSP + the sync codec/fit (node --test on src/features/**/*.test.ts)
...
- Optional **cross-device sync** (`src/features/sync/`) rides `browser.storage.sync` — no server, no ID, no cookies; the browser vendor's sync carries the bytes. The backup snapshot is compacted ([sync-snapshot.ts](src/features/sync/persist/sync-snapshot.ts): chord charts as centisecond delta arrays, ~12 B/segment instead of ~90), gzipped + base64 + chunked into `nbn.meta` / `nbn.0…` items of ≤ 8 KB ([sync-blob.ts](src/features/sync/persist/sync-blob.ts); `meta.hash` over the base64 detects a torn read from a concurrent writer), and trimmed by priority to the 100 KB quota ([fit.ts](src/features/sync/persist/fit.ts): favorites + settings never trimmed; then Recent newest-first, other tracks, charts last). Last-write-wins on `exportedAt`; remote changes arrive via `storage.sync.onChanged`; applying **merges Recent and the per-track records** ([apply.ts](src/features/sync/persist/apply.ts) → [merge.ts](src/features/sync/persist/merge.ts)) so an item the sender trimmed is never deleted on the receiver — a real deletion travels as a dated **deletion record** ([core/persist/deletions.ts](src/core/persist/deletions.ts), one flat `key → when` map written by `removeTrackData` / `removeHistoryEntry` / `clearHistory`, never trimmed, TTL 30 d + cap 200) that kills copies not edited since. The store ([sync.svelte.ts](src/features/sync/panel/sync.svelte.ts)) keeps three distinct hashes (raw-local for change detection, fitted-content for cross-device equality, blob for integrity/echo), spaces pushes ≥ 30 s apart (writes are capped at 120/min), and re-seeds an area that stays `torn` for 30–60 s (two devices' writes merged per key — nothing can read it, so nothing would ever push over it) under its *last-sync* clock with `repaired` set, so a device that synced something newer pushes that back instead of applying the stale re-seed. Per-track records with no content are deleted rather than written ([track-content.ts](src/core/model/track-content.ts)); `pruneEmptyTrackData` sweeps legacy ones at panel boot. The pure `persist/` modules use relative `.ts` imports and run under `node --test`.
