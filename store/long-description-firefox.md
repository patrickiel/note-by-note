...
- Optional cross-device sync copies that setup to your other browsers through Firefox Sync — no server, no account.
...
- The extension makes no network requests of its own. Cross-device sync uses Firefox's own sync storage — no server of ours — is on by default, switches off in Settings, and carries no audio: just your settings, markers, loops, snippets, and the page URLs and titles of the tracks in your library.
